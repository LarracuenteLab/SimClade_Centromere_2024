...
pdf("SechCen4.pdf",,width = 15, height = 7)
...
kp <- plotKaryotype(genome = custom.genome, cytobands = custom.cytobands, ideogram.plotter = NULL,label=NULL,cex=0.7, plot.params = pp, chromosomes=chr)

kpAddBaseNumbers(kp,tick.dist=10000,tick.len=6, cex=1.5, digits=3, units="kb" )
kpAddCytobandsAsLine(kp,color.table=vv,lwd=25)

kp<- kpPlotBigWig(kp, data.panel = 1, r0=0.06, r1=1, data="Coverage/Sech_CID1_bwa_q0_RPM.bw", ymax=800, col="grey",border=NA)
kp<- kpPlotBigWig(kp, data.panel = 1, r0=0.06, r1=1, data="Coverage/Sech_CID1_bwa_q30_RPM.bw", ymax=800, col="black",border=NA)
...
pdf("SechCenX.pdf",,width = 15, height = 7)
pp <- getDefaultPlotParams(plot.type=1)
pp$leftmargin <- 0.15
pp$topmargin <- 20
pp$bottommargin <- 20
...
kp<- kpPlotBigWig(kp, data.panel = 1, r0=0.06, r1=1, data="Coverage/Sech_CID1_bwa_q0_RPM.bw", ymax=400, col="grey",border=NA)
kp<- kpPlotBigWig(kp, data.panel = 1, r0=0.06, r1=1, data="Coverage/Sech_CID1_bwa_q30_RPM.bw", ymax=400, col="black",border=NA)

kpAxis(kp, ymin=0 , ymax=400, r0=0.06, r1=1, numticks = 2,cex=1.8)
kpAddLabels(kp, labels = "Cenp-A enrichment (RPM)", srt=90, pos=3, cex=2, label.margin = 0.035)
kpPlotRegions(kp, data=peaksq0, data.panel = 1, r0=0,r1=0.025,avoid.overlapping=FALSE,col="grey",border=NA)
kpPlotRegions(kp, data=peaksq30, data.panel = 1, r0=0,r1=0.025,avoid.overlapping=FALSE,col="black",border=NA)
dev.off()
